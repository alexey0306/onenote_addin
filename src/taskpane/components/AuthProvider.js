...
export default C =>
    class AuthProvider extends Component {
        ...
        async onListNotebooks() {
            const tokenResponse = await this.acquireToken(
                GRAPH_REQUESTS.LOGIN,
                useRedirectFlow
            ).catch(e => {
                this.setState({
                    error: "Unable to acquire access token for listing notebooks"
                });
            });

            if (tokenResponse) {
                return this.listNotebooks(tokenResponse.accessToken);
            }
        }

        async listNotebooks(accessToken){
            const notebooks = await fetchMsGraph(
                GRAPH_ENDPOINTS.NOTEBOOKS,
                accessToken
            ).catch(() => {
                this.setState({
        ...
        render() {
            return (
                <C
                    ...
                    onListNotebooks={() => this.onListNotebooks()}
                />
            );
        }
    };
